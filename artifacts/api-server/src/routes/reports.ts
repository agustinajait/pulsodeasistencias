...
router.get("/children/:id/reports", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT id, child_id AS "childId", period, eco_number AS "ecoNumber", lider, facilitadora, hitos, observaciones, created_at AS "createdAt", updated_at AS "updatedAt"
...
router.post("/children/:id/reports", async (req, res) => {
  try {
    ...
    const childId = parseInt(req.params.id);
    if (isNaN(childId)) { res.status(400).json({ error: "Invalid id" }); return; }
    const { period, ecoNumber, lider, facilitadora, hitos, observaciones } = req.body as Record<string, any>;
    if (!period) { res.status(400).json({ error: "period is required" }); return; }
    const result = await pool.query(
      `INSERT INTO child_reports (child_id, period, eco_number, lider, facilitadora, hitos, observaciones)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING id, child_id AS "childId", period, eco_number AS "ecoNumber", lider, facilitadora, hitos, observaciones, created_at AS "createdAt", updated_at AS "updatedAt"`,
      [childId, period, ecoNumber ?? null, lider ?? null, facilitadora ?? null, JSON.stringify(hitos ?? {}), observaciones ?? null]
    );
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE child_reports
       ...
       WHERE id = $1
       RETURNING id, child_id AS "childId", period, eco_number AS "ecoNumber", lider, facilitadora, hitos, observaciones, created_at AS "createdAt", updated_at AS "updatedAt"`,
      [reportId, period ?? null, ecoNumber ?? null, lider ?? null, facilitadora ?? null, hitos ? JSON.stringify(hitos) : null, observaciones ?? null]
    );
